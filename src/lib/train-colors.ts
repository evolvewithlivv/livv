...
import type { SplitTarget } from "./train-data";

export type SplitId = "ppl" | "upper-lower" | "bro" | "full" | "hybrid";

export type SplitDay = { id: string; day: string; label: string; focus: Focus | null; target?: SplitTarget; muscles: string; rest?: boolean };
export type SplitDef = { id: SplitId; name: string; line: string; detail: string; days: SplitDay[]; color: string };

export const SPLITS: SplitDef[] = [
  { id: "ppl", name: "Push / Pull / Legs", line: "Push one day. Pull the next. Legs after that.", detail: "A repeatable 6-day structure built around movement patterns and recovery.", color: "#FF4D6D", days: [
  ...
  { id: "upper-lower", name: "Upper / Lower", line: "Upper one day. Lower the next. Repeat.", detail: "Simple frequency without making every session a full-body marathon.", color: "#4C8DFF", days: [
  ...
  { id: "bro", name: "Bro Split", line: "One major focus per training day. Five days. Recover hard.", detail: "Each day has a primary muscle-group mission instead of bundling everything together.", color: "#F5C542", days: [
    ...
    { id: "bro-thu", day: "THU", label: "Arms", focus: "Push", target: "Arms", muscles: "Biceps · triceps · forearms" },
  ...
  { id: "full", name: "Full Body", line: "Train the whole body each session. Recover between sessions.", detail: "A flexible structure for people who want fewer training days without skipping muscle groups.", color: "#3DDC97", days: [
  ...
  { id: "hybrid", name: "Hybrid", line: "Strength, engine, and trunk work rotated across the week.", detail: "A balanced structure for people who want lifting plus conditioning in one system.", color: "#B45CFF", days: [
...
export const FOCUS_COLOR: Record<Focus, string> = { "Full Body": "#1EC8A5", "Upper Body": "#9B7CFF", "Lower Body": "#D4A017", Push: "#FF7A1A", Pull: "#5B8CFF", Legs: "#C8F542", Core: "#86E35A", Cardio: "#FF3B6B" };
...
export function chipStyle(hex: string, on: boolean) { return { borderColor: on ? hex : `${hex}55`, background: on ? `${hex}2E` : `${hex}14`, boxShadow: on ? `0 0 22px ${hex}55` : undefined, color: on ? "#fff" : "rgba(255,255,255,0.86)" } as const; }
