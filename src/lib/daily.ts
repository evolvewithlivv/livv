...
const DROP_POOL: { weight: number; drop: DailyDrop }[] = [
  ...
  { weight: 8, drop: { id: "pack-ticket", kind: "pack_ticket", name: "Pack Ticket", description: "An extra pull. Claim it on the Packs tab.", icon: "▣", embers: 20, xp: 20 } },
  { weight: 8, drop: { id: "double-xp", kind: "double_xp_day", name: "2× XP Day", description: "Tomorrow’s actions grant double XP until midnight.", icon: "⚡", embers: 25, xp: 30 } },
  { weight: 6, drop: { id: "frame-signal", kind: "frame", name: "Frame · Signal", description: "A profile frame. Equip it from Profile when ready.", icon: "◎", embers: 30, xp: 20, meta: "signal" } },
  { weight: 5, drop: { id: "frame-ascent", kind: "frame", name: "Frame · Ascent", description: "Rare profile frame for people who finish Dailies.", icon: "◉", embers: 40, xp: 25, meta: "ascent" } },
  { weight: 4, drop: { id: "artifact-ledger", kind: "artifact", name: "Artifact · The Ledger", description: "A collectible mark of continuity. Yours permanently.", icon: "◈", embers: 50, xp: 40, meta: "ledger" } },
  { weight: 1, drop: { id: "golden-drop", kind: "embers", name: "Golden Drop", description: "The uncommon pull. Consistency made visible.", icon: "✹", embers: 150, xp: 100 } },
];
...
export function loadBuffs(): DailyBuffs {
  ...
  try {
    ...
    return { doubleXpUntil: null, frames: [], activeFrame: null, artifacts: [], packTickets: 0, ...JSON.parse(raw) } as DailyBuffs;
  } catch {
    return { doubleXpUntil: null, frames: [], activeFrame: null, artifacts: [], packTickets: 0 };
  }
}
...
export function dailyTasks(date = new Date()): DailyTask[] {
  try {
    return adaptiveDailyTasks(date, loadRecord());
  } catch {
...
export function loadDailyState(date = new Date()) {
  ...
  const dropRevealed = data.dropRevealed && typeof data.dropRevealed === "object" ? (data.dropRevealed as Record<string, boolean>) : {};
  return {
    key,
...
export function completeDailyTask(id: DailyTask["id"], date = new Date()) {
  ...
  if (!completed.includes(token)) {
    if (id === "mind") {
      ...
      if (!journal.some((j) => j.key === key && j.answer.trim().length > 0)) return loadDailyState(date);
    }
    const task = dailyTasks(date).find((item) => item.id === id);
    if (task) {
      logCustomAction({ title: `Daily · ${task.title}`, pillar: task.pillar, size: task.xpSize });
    }
...
function applyDropEffects(drop: DailyDrop) {
  ...
  if (drop.kind === "frame" && drop.meta && !buffs.frames.includes(drop.meta)) buffs.frames.push(drop.meta);
  if (drop.kind === "artifact" && drop.meta && !buffs.artifacts.includes(drop.meta)) buffs.artifacts.push(drop.meta);
...
export function claimDailyDrop(date = new Date()) {
  ...
  const key = dayKey(date);
  const claimed =
    data.claimed && typeof data.claimed === "object" ? { ...(data.claimed as Record<string, boolean>) } : {};
