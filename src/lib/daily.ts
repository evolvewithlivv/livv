...
const DROP_POOL: { weight: number; drop: DailyDrop }[] = [
  ...
  },
  {
    ...
    drop: {
      ...
      kind: "pack_ticket",
      name: "Pack Ticket",
      description: "One free pack open on the Shop.",
      icon: "▣",
      embers: 5,
      xp: 10,
    },
  },
  {
    ...
    drop: {
      id: "double-xp",
      kind: "double_xp_day",
      name: "Double XP Window",
      description: "Next 24 hours of actions count double toward Evolution.",
      icon: "⚡",
      embers: 20,
      xp: 20,
      meta: "1",
  ...
  },
  {
    ...
    drop: {
      ...
      kind: "frame",
      name: "Ember Frame",
      description: "A profile frame unlocked by finishing the Daily.",
      icon: "◎",
      embers: 10,
      xp: 15,
      meta: "frame-ember",
  ...
  },
  {
    ...
    drop: {
      ...
      kind: "artifact",
      name: "Artifact Shard",
      description: "A collectible for the long game.",
      icon: "◆",
      embers: 25,
      xp: 30,
      meta: "shard-1",
  ...
  },
  {
    ...
    drop: {
      ...
      kind: "accent",
      name: "Accent Shift",
      description: "A temporary accent tone for your profile.",
      icon: "◌",
      embers: 15,
      xp: 20,
      meta: "#67d8ff",
...
export function dailyTasks(date = new Date()): DailyTask[] {
  // Adaptive Daily v1: bias set from LivvRecord (weak pillar + recent inactivity).
  // Falls back to pure day-hash if adaptive layer throws.
  try {
...
export function completeDailyTask(id: DailyTask["id"], date = new Date()) {
  ...
  const completed = Array.isArray(data.completed)
    ...
    : [];
  const stamp = `${key}:${id}`;
  if (completed.includes(stamp)) return loadDailyState(date);
  completed.push(stamp);
...
export function consumePackTicket(): boolean {
  const buffs = loadBuffs();
  if (buffs.packTickets <= 0) return false;
  buffs.packTickets -= 1;
  saveBuffs(buffs);
  return true;
}
