...
export async function recordPackPurchase(row:{userId:string;stripeSessionId:string;stripeCustomerId?:string|null;grade:number;quantity:number;}){const admin=getSupabaseAdmin();if(!admin)throw new Error("Supabase admin client unavailable");const{error}=await admin.from("pack_purchases").upsert({user_id:row.userId,stripe_session_id:row.stripeSessionId,stripe_customer_id:row.stripeCustomerId||null,grade:row.grade,quantity:row.quantity,status:"paid"},{onConflict:"stripe_session_id",ignoreDuplicates:true});if(error)throw error;}
