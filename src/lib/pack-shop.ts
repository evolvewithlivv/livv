...
export function grantPurchasedPack(grade: PackGrade, qty = 1): PackState {
  const state = loadPacks(); const now = Date.now(); const n = Math.max(1, Math.min(10, qty));
  for (let i = 0; i < n; i++) state.pending.push({ id: `buy_${grade}_${now}_${i}_${Math.random().toString(36).slice(2, 6)}`, grade, grantedAt: now });
  savePacks(state); return state;
}
...
export async function buyPack(grade: PackGrade): Promise<{ ok: boolean; error?: string; local?: boolean }> {
  ...
  if (isStripeConfigured()) {
    try {
      const res = await fetch("/api/stripe/checkout", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ kind: "pack", grade, qty: 1, username: me.username }) });
      const data = (await res.json()) as { url?: string; error?: string };
      if (!res.ok || !data.url) return { ok: false, error: data.error || "Checkout unavailable" };
      window.location.href = data.url; return { ok: true };
    } catch { return { ok: false, error: "Network error starting checkout" }; }
  }
  grantPurchasedPack(grade); return { ok: true, local: true };
}
