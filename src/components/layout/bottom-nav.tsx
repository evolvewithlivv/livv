...
export function BottomNav() {
  ...
  return <nav className="relative z-50 shrink-0 px-3 pb-[max(.6rem,env(safe-area-inset-bottom))] pt-2"><div className="mx-auto flex max-w-lg items-center justify-around rounded-[27px] border border-white/[.1] bg-[#090b10]/75 px-1.5 py-1.5 shadow-[0_22px_70px_rgba(0,0,0,.58),inset_0_1px_0_rgba(255,255,255,.08)] backdrop-blur-2xl backdrop-saturate-150">{NAV_ITEMS.map((item) => { const active = pathname === item.href || (item.href !== "/home" && pathname.startsWith(item.href)); return <Link key={item.href} href={item.href} aria-current={active ? "page" : undefined} className={cn("group relative flex min-w-[60px] flex-col items-center gap-1 rounded-[20px] px-2.5 py-2.5 transition-all duration-300", active ? "text-white" : "text-white/30")}>{active && <span className="absolute inset-0 rounded-[20px] border border-livv-accent/15 bg-livv-accent/[.11] shadow-[0_0_30px_rgb(var(--livv-accent)/.14)]" />}<span className="absolute top-1 h-0.5 w-5 rounded-full bg-livv-accent opacity-0 shadow-[0_0_12px_rgb(var(--livv-accent)/.9)] transition-opacity group-hover:opacity-50"/><span className="relative transition-transform duration-300 group-hover:-translate-y-0.5"><item.icon active={active}/></span><span className={cn("relative text-[8px] font-medium uppercase tracking-[.14em]", active ? "text-livv-accent-soft" : "text-white/25")}>{item.label}</span></Link>; })}</div></nav>;
}
...
function PacksIcon({active}:{active:boolean}){return <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={active?2.2:1.7}><rect x="6" y="3" width="12" height="18" rx="2"/><path d="M6 7h12M6 17h12" strokeLinecap="round"/></svg>}
