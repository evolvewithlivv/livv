...
export default function AuthPage() {
  ...
  const afterAuth = () => router.replace(isOnboardingComplete() ? "/home" : "/onboarding");
  ...
  const submitPhone = () => void run(async () => {
    ...
    setPhoneOtp("");
    setNotice("We sent an 8-digit code to your phone.");
    startCooldown();
  });

  const verifyEmail = () => void run(async () => { await verifyEmailAuth(email, emailOtp); afterAuth(); });
  const verifyPhone = () => void run(async () => { await verifyPhoneAuth(phone, phoneOtp); afterAuth(); });

  const resendEmail = () => void run(async () => {
    await startEmailAuth(email);
    setNotice("A new email is on its way. Enter the 8-digit code from that message.");
    startCooldown();
  });
  ...
  const resendPhone = () => void run(async () => {
    const result = await startPhoneAuth(phone);
    if (result.phone) setPhone(result.phone);
    setNotice("A new 8-digit code is on its way.");
    startCooldown();
  });
    ...
    <main className="relative min-h-dvh overflow-hidden bg-[#050505] text-white">
      ...
      <div className="relative z-10 mx-auto flex min-h-dvh w-full max-w-md flex-col justify-center px-6 py-12">
        <div className="mb-10 flex flex-col items-center">
          ...
          <p className="mt-2 text-center text-[13px] text-white/40">Email or phone. 8-digit codes only. No social login.</p>
        </div>

        {mode === "choose" && <div className="space-y-3">
        ...
        {mode === "email" && <div className="space-y-4">
          {!emailPending ? <form className="space-y-4" onSubmit={(e) => { e.preventDefault(); submitEmail(); }}>
            ...
            <Button className="w-full" disabled={busy || !email.includes("@")} type="submit">{busy ? "Sending…" : "Send 8-digit code"}</Button>
        ...
        {mode === "phone" && <div className="space-y-4">
          {!phonePending ? <form className="space-y-4" onSubmit={(e) => { e.preventDefault(); submitPhone(); }}>
            <Field label="Phone" value={phone} onChange={setPhone} type="tel" autoComplete="tel" inputMode="tel" placeholder="+15551234567" />
            <p className="text-[11px] text-white/30">Include country code. US numbers without + are assumed +1.</p>
            <Button className="w-full" disabled={busy || phone.replace(/\D/g, "").length < 7} type="submit">{busy ? "Sending…" : "Send 8-digit code"}</Button>
