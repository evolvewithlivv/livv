...
export default function AuthPage() {
  ...
  return (
    <main className="relative min-h-dvh overflow-hidden bg-[#050505] text-white">
      <div aria-hidden className="pointer-events-none absolute inset-0 bg-[radial-gradient(ellipse_at_top,rgba(var(--livv-accent)/0.12),transparent_55%)]" />
      <div className="relative z-10 mx-auto flex min-h-dvh w-full max-w-md flex-col justify-center px-6 py-12">
        <div className="mb-10 flex flex-col items-center">
          ...
          <p className="mt-2 text-center text-[13px] text-white/40">Email or phone. No passwords. No social login.</p>
        </div>
          ...
          <div className="space-y-4">
            {!phonePending ? (
              <form className="space-y-4" onSubmit={(e) => { e.preventDefault(); submitPhone(); }}>
                ...
                <Button className="w-full" disabled={busy || phone.replace(/\D/g, "").length < 7} type="submit">{busy ? "Sending…" : "Send 6-digit code"}</Button>
              </form>
            ) : (
              <form className="space-y-4" onSubmit={(e) => { e.preventDefault(); verifyPhone(); }}>
...
function Notice({ children }: { children: string }) {
  return <div className="rounded-2xl border border-white/10 bg-white/[0.04] p-4 text-[12px] leading-5 text-white/60">{children}</div>;
}
function ErrorMessage({ children }: { children: string }) {
  return <p className="text-center text-[13px] text-red-400">{children}</p>;
}
function Back({ onClick }: { onClick: () => void }) {
  return <button type="button" onClick={onClick} className="w-full py-2 text-center text-[13px] text-white/35">Back</button>;
}
