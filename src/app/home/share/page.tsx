  ...
  const share = async () => {
    ...
    try {
      const blob = mode === "profile"
        ? await renderProfileShareCard({ displayName: me.displayName || "LIVV member", username: me.username || "livv", level: rec.level, evolutionName: evolutionTitle(rec.level), streak: rec.streak, tierLabel: tier.name, tierColor: color, embers: me.embers, badges, workoutsCompleted: rec.workoutsCompleted })
