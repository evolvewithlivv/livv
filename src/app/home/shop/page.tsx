...
export default function ShopPage() {
  ...
  async function handlePackBuy(grade: PackGrade) {
    ...
    try {
      ...
      if (out.local) { feedback("unlock"); sync(); }
  ...
  return (
    <main className="livv-page relative min-h-full overflow-hidden text-white">
      <div className="relative z-10 mx-auto max-w-xl px-5 pt-7">
        ...
        <section className="mt-8">
          <div className="mb-4 flex items-end justify-between px-1">
            <div>
              <p className="text-[9px] font-semibold uppercase tracking-[0.3em] text-white/30">Essentials</p>
              ...
              <a key={product.href} href={product.href} className="livv-glass group overflow-hidden rounded-[28px] transition active:scale-[0.98]">
                ...
                <div className="p-4">
                  ...
                  <div className="mt-4 flex items-center justify-between">
                    ...
                    <span className="flex h-8 w-8 items-center justify-center rounded-full border border-white/10 bg-white/[0.05] text-white/60 group-hover:border-[#ff9f43]/35 group-hover:text-[#ff9f43]"><ArrowUpRight size={15} /></span>
                  </div>
                </div>
              </a>
            ))}
        ...
        <section className="mt-10">
          <div className="livv-glass overflow-hidden rounded-[32px] p-5">
            <div className="flex items-start justify-between gap-4">
              <div>
                <p className="text-[9px] font-semibold uppercase tracking-[0.3em] text-[#ff9f43]">Evolution packs</p>
                <h2 className="font-display mt-2 text-[27px] font-semibold tracking-[-0.04em]">Collect the signal.</h2>
                <p className="mt-2 max-w-[34ch] text-[12px] leading-relaxed text-white/40">Packs now live inside Shop. Membership can grant packs automatically; paid pulls are optional.</p>
              </div>
            ...
            <div className="mt-5 rounded-[24px] border border-white/[0.07] bg-white/[0.025] p-4">
              <div className="flex items-center justify-between gap-3">
                ...
                <div className="text-right">
                  ...
                  <p className="mt-1 text-sm font-semibold">{stats.uniqueCount}<span className="text-white/25">/{stats.catalogSize}</span></p>
                </div>
              </div>
              ...
              {ready && (
                <div className="mt-5 flex gap-4 overflow-x-auto pb-1 scrollbar-none">
                  {pending.map((pack) => (
                    <button key={pack.id} type="button" onClick={() => { feedback("tick"); setOpening(pack); }} className="shrink-0 text-center">
                      <PackFoil grade={pack.grade} size="sm" pulse />
                      <p className="mt-2 text-[9px] uppercase tracking-[0.16em] text-white/40">{GRADE_META[pack.grade].name.replace(" Pack", "")}</p>
                    </button>
                  ))}
                </div>
              )}
            ...
            <div className="mt-5 space-y-2.5">
              {([1, 2, 3, 4] as PackGrade[]).map((grade) => {
                ...
                return (
                  <div key={grade} className="rounded-[24px] border border-white/[0.07] bg-white/[0.02] p-4">
                    <div className="flex items-center gap-3">
                      <PackFoil grade={grade} size="sm" />
                      <div className="min-w-0 flex-1">
                        <div className="flex items-baseline justify-between gap-3"><p className="text-[15px] font-semibold">{meta.name}</p><p className="text-[15px] font-semibold">{shop.price}</p></div>
                        <p className="mt-1 text-[11px] leading-relaxed text-white/35">{meta.subtitle} · {shop.value}</p>
                      </div>
                    </div>
                    <button type="button" disabled={buying !== null} onClick={() => handlePackBuy(grade)} className="mt-3 w-full rounded-full bg-white py-2.5 text-[11px] font-semibold uppercase tracking-[0.12em] text-black disabled:opacity-50">
        ...
        <Link href="/home/vault" className="livv-glass mt-4 flex items-center justify-between rounded-[28px] px-5 py-4 transition active:scale-[0.99]">
          <div><p className="text-[9px] font-semibold uppercase tracking-[0.28em] text-[#ff9f43]">Your archive</p><p className="mt-1 text-[13px] font-medium text-white/75">Open the vault</p></div>
      ...
      {me && <CardFace card={undefined as never} size="sm" />}
    </main>
  );
}
