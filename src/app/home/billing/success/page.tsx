...
export default function BillingSuccessPage() {
  ...
  useEffect(()=>{ const sessionId=params.get("session_id"); if(!sessionId){setStatus("error");setMessage("Missing checkout session.");return;} let cancelled=false;
    (async()=>{try{const client=isSupabaseConfigured()?getSupabaseBrowserClient():null; const {data:sd}=await client?.auth.getSession()??{data:{session:null}}; const token=sd.session?.access_token;
      ...
      applyStripeEntitlement({tier:data.tier,customerId:data.customerId,subscriptionId:data.subscriptionId});invalidateServerEntitlementCache();void hydrateServerEntitlement();await syncCloudMemberState().catch(()=>undefined);feedback("unlock");
      if(!cancelled){setStatus("ok");setMessage(`${label(data.tier)} is active on your account.`);window.setTimeout(()=>router.replace("/home/profile"),1600);}
