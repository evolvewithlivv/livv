...
function ActionCard({ action, complete, onOpen }: { action: Action; complete: boolean; onOpen: () => void }) {
  return <button type="button" onClick={onOpen} className="relative min-h-[126px] overflow-hidden rounded-[24px] border p-4 text-left transition hover:border-white/20 active:scale-[0.98]" style={{ borderColor: complete ? `${action.color}55` : "rgba(255,255,255,.10)", background: complete ? `linear-gradient(145deg, ${action.color}0A, rgba(11,13,16,.94))` : "linear-gradient(145deg, rgba(17,19,24,.92), rgba(8,10,13,.96))" }}>
    <span className="absolute right-4 top-4 h-2 w-2 rounded-full" style={{ background: action.color }} />
...
export default function HomePage() {
  ...
  useEffect(() => { pull(); setQuote(quoteForSession()); const timer = window.setInterval(() => setNow(new Date()), 30000); for (const event of ["livv-identity", "livv-record", "livv-daily", "livv-billing"]) window.addEventListener(event, pull); return () => { window.clearInterval(timer); for (const event of ["livv-identity", "livv-record", "livv-daily", "livv-billing"]) window.removeEventListener(event, pull); }; }, []);
  ...
  const onCheckIn = () => { if (checkedIn) return; const result = checkInRecord(); if (result.already) return; feedback("checkin"); addEmbers(10 * tier.multiplier + (result.emberBonus || 0)); if (canClaimPacks(getEffectiveTier())) claimPacksIfDue(getEffectiveTier()); pull(); };
  return <main className="livv-page relative min-h-full overflow-hidden pb-16 text-white"><div className="relative z-10 mx-auto max-w-xl px-5 pt-5">
    <section className="livv-glass relative overflow-hidden rounded-[34px] px-6 pb-7 pt-6"><div className="absolute inset-x-0 top-0 h-px bg-gradient-to-r from-transparent via-livv-accent/60 to-transparent"/><div className="flex items-start justify-between gap-5"><div className="min-w-0"><p className="text-[10px] font-semibold uppercase tracking-[0.28em] text-livv-accent-soft">{daily?.world.focus || "Today"}</p><h1 className="font-display mt-3 text-[31px] font-semibold leading-[0.98] tracking-[-0.04em]">{daily?.world.line || `Welcome back, ${me.displayName || "member"}.`}</h1></div><div className="shrink-0 text-right"><p className="text-[10px] uppercase tracking-[0.2em] text-white/45">Streak</p><p className="font-display mt-1 text-3xl font-semibold">{rec.streak}</p><p className="text-[10px] uppercase tracking-[0.18em] text-white/45">days</p></div></div>{quote && <blockquote className="relative mt-7 border-l-2 border-livv-accent/50 pl-4"><p className="font-display text-[17px] font-medium leading-snug text-white/90">“{quote.text}”</p><footer className="mt-2 text-[10px] uppercase tracking-[0.2em] text-white/45">{quote.author}</footer></blockquote>}</section>
    <section className="mt-5 rounded-[28px] border border-white/10 bg-white/[0.025] p-5"><div className="flex items-start justify-between gap-4"><div><p className="text-[10px] font-semibold uppercase tracking-[0.24em] text-livv-accent-soft">Next move</p><h2 className="font-display mt-2 text-[23px] font-semibold">{loop.move.title}</h2><p className="mt-1 text-[12px] leading-relaxed text-white/60">{loop.move.reason}</p></div><Link href={loop.move.href} className="shrink-0 rounded-full bg-white px-4 py-2.5 text-[10px] font-bold text-black">{loop.move.cta}</Link></div><p className="mt-4 border-t border-white/10 pt-4 text-[11px] leading-relaxed text-white/45">{loop.status}</p></section>
