  ...
  return (
    <main className="relative min-h-full overflow-hidden pb-28">
      <div className="pointer-events-none fixed inset-0">
        ...
        <div
          className="absolute left-1/2 top-[-90px] h-[480px] w-[480px] -translate-x-1/2 rounded-full"
          style={{ background: `radial-gradient(circle, ${tc.glow}, transparent 68%)` }}
        />
        <AmbientField />
      </div>
      ...
      <div className="relative z-10 mx-auto max-w-lg px-5 pt-6">
        <header className="flex items-center justify-between">
          ...
          <Link
            href="/home/settings"
            aria-label="Settings"
            className="grid h-10 w-10 place-items-center rounded-full border border-white/10 bg-white/[0.04] text-white/55 backdrop-blur-xl"
          >
            <Settings2 size={17} />
          </Link>
        </header>
        ...
        <section className="relative mt-8 overflow-hidden rounded-[32px] border border-white/10 bg-white/[0.035] px-6 pb-6 pt-7 text-center shadow-2xl backdrop-blur-xl">
          <div
            ...
            style={{ background: `linear-gradient(90deg, transparent, ${tc.hex}, transparent)` }}
          />
          <button
            type="button"
          ...
          <div
            className="mt-3 inline-flex items-center gap-2 rounded-full border px-3 py-1.5 text-[10px] font-semibold uppercase tracking-[0.18em]"
            style={{
              color: tc.hex,
              borderColor: `${tc.hex}44`,
              background: `${tc.hex}12`,
            }}
          >
            <Crown size={12} /> {tc.label}
          </div>
          {me.bio && (
            <p className="mx-auto mt-4 max-w-[30ch] text-[14px] leading-relaxed text-white/50">{me.bio}</p>
          )}
        ...
        <section className="mt-5 rounded-[28px] border border-white/10 bg-white/[0.03] p-5 backdrop-blur-xl">
          ...
          <div className="mt-5 h-2 overflow-hidden rounded-full bg-white/8">
            <div
              className="h-full rounded-full bg-livv-accent shadow-[0_0_18px_rgb(var(--livv-accent)/0.65)] transition-all"
              style={{ width: `${xpPct}%` }}
            />
          </div>
          <div className="mt-4 flex items-center justify-between text-[11px] text-white/30">
            <span>{xpPct}% to next level</span>
            <Link href="/home/progress" className="font-medium text-livv-accent-soft">
              Open evolution <ArrowRight size={12} className="ml-1 inline" />
            </Link>
          </div>
        </section>
        ...
        <div className="mt-4 grid grid-cols-2 gap-3">
          <Link
            href="/home/progress"
            className="group rounded-[24px] border border-white/8 bg-white/[0.03] p-4 transition hover:bg-white/[0.06]"
          >
            ...
            <ChevronRight
              size={15}
              className="mt-3 text-white/25 transition group-hover:translate-x-1"
            />
          </Link>
          <Link
            href="/home/vault"
          ...
          >
            ...
            <p className="mt-1 text-[11px] text-white/35">
              {vault.uniqueCount}/{vault.catalogSize} collected
            </p>
            <ChevronRight
              size={15}
              className="mt-3 text-white/25 transition group-hover:translate-x-1"
        ...
        <section className="mt-12">
          ...
          <p className="mt-2 max-w-[34ch] text-[13px] leading-relaxed text-white/38">
            More access. More compounding. Pick the level that matches where you&apos;re going.
          </p>
          ...
          <div className="mt-5 space-y-3">
            {TIERS.map((t) => {
              ...
              return (
                ...
                >
                  <div
                    className="relative overflow-hidden rounded-[26px] border p-5 transition active:scale-[0.99]"
                    style={{
                      borderColor: active
                        ? `${color.hex}88`
                        : t.featured
                      ...
                      background: active
                        ? `linear-gradient(135deg, ${color.hex}20, rgba(255,255,255,0.035))`
                        : t.featured
                          ? `linear-gradient(135deg, ${color.hex}0f, rgba(255,255,255,0.025))`
                          : "rgba(255,255,255,0.025)",
                      boxShadow: active ? `0 0 32px ${color.glow}` : undefined,
                    }}
                  >
                    {t.featured && (
                      <div
                        className="absolute right-4 top-4 rounded-full px-2.5 py-1 text-[9px] font-bold uppercase tracking-[0.16em]"
                        style={{ color: color.hex, background: `${color.hex}16` }}
                      >
                        Most chosen
                      </div>
                    )}
                    <div className="flex items-start gap-4">
                      <div
                        ...
                        style={{ background: `${color.hex}16`, color: color.hex }}
                      >
                        {t.id === "spark" ? (
                          <Sparkles size={19} />
                        ) : t.id === "rise" ? (
                      ...
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center gap-2 pr-20">
                          <p
                            ...
                            style={{
                              color: active || t.featured ? color.hex : undefined,
                            }}
                          >
                            {t.name}
                          </p>
                          {active && (
                            <span className="rounded-full bg-white/10 px-2 py-0.5 text-[8px] font-bold uppercase tracking-[0.12em] text-white/55">
                              Current
                            </span>
                          )}
                        </div>
                        <div className="mt-0.5 flex items-baseline gap-1">
                          ...
                          <span className="ml-2 text-[10px] font-medium" style={{ color: color.hex }}>
                            {t.multiplier}x Embers
                          </span>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="mt-5 flex items-center justify-between border-t border-white/8 pt-4">
                      ...
                      <span className="grid h-8 w-8 place-items-center rounded-full border border-white/10 bg-white/[0.04] text-white/50">
                        <ArrowRight size={14} />
                      </span>
                    </div>
                  </div>
          ...
          {entitlements.stripeCustomerId && (
            <button
              ...
              className="mt-4 w-full rounded-full border border-white/10 py-3 text-[13px] text-white/50"
            >
              Manage subscription
            </button>
          )}
          ...
          <p className="mt-4 text-center text-[11px] leading-relaxed text-white/25">
            {stripeLive
              ? "Paid tiers go through Stripe Checkout. You can cancel anytime in Manage subscription."
              : "Add Stripe keys in Vercel to enable real checkout. Demo unlock remains for testing."}
          </p>
        </section>

        <section className="mt-10 rounded-[26px] border border-white/8 bg-white/[0.025] p-5">
          <div className="flex items-center gap-3">
            <Sparkles size={17} className="text-livv-accent-soft" />
            <div>
        ...
        <div className="mt-10 flex items-center justify-between border-t border-white/8 pt-6">
          <Link href="/home/messages" className="text-[13px] text-livv-accent-soft">
            Messages →
          </Link>
          <Link href="/home/evala" className="text-[13px] text-white/35">
            Ask Evala →
          </Link>
        </div>
      </div>
      ...
      {picked && (
        <div
          className="fixed inset-0 z-[70] flex items-end bg-black/80 backdrop-blur-md"
          onClick={() => {
            setPicked(null);
            setBillingNote(null);
          }}
        >
          <div
            className="w-full rounded-t-[32px] border-t border-white/10 bg-[#0b0d11] p-6 pb-10 shadow-2xl"
          ...
          >
            ...
            <div className="flex items-center gap-4">
              <div
                ...
                style={{
                  color: tierColor(picked).hex,
                  background: `${tierColor(picked).hex}16`,
                }}
              >
                <Crown size={20} />
              </div>
              ...
              <p className="ml-auto text-lg font-semibold">
                ...
                <span className="text-xs text-white/30">
                  {getTier(picked).cadence !== "forever" ? getTier(picked).cadence : ""}
                </span>
              </p>
            </div>
            ...
            {billingNote && (
              <p className="mt-4 rounded-2xl border border-amber-300/15 bg-amber-300/5 p-3 text-[12px] leading-relaxed text-amber-200/80">
                {billingNote}
              </p>
            )}
            <div className="mt-7 flex flex-col gap-2">
              <button
                type="button"
                disabled={checkoutBusy || me.tier === picked}
              ...
              {!isDemoUnlock() && picked !== "spark" && !stripeLive && (
                <button
                  ...
                  className="py-2 text-[11px] text-white/20"
                >
...
function Stat({ value, label, icon }: { value: string; label: string; icon: ReactNode }) {
  return (
    <div className="flex flex-col items-center gap-1">
      <span className="flex items-center gap-1 text-[10px] uppercase tracking-[0.13em] text-white/25">
        {icon}
        {label}
      </span>
