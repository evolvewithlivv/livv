...
export default function ArticlePage(){
  ...
  useEffect(()=>{if(article)setSaved(readSaved().includes(article.slug));},[article]);
  ...
  const toggleSaved=()=>{const current=readSaved();const nextSaved=current.includes(article.slug)?current.filter(x=>x!==article.slug):[...current,article.slug];try{window.localStorage.setItem(SAVED_KEY,JSON.stringify(nextSaved));}catch{}setSaved(nextSaved.includes(article.slug));};
