...
import { SOUND_LIBRARY, canEditPost, createPost, deletePost, fileToPostPhoto, formatSocialTime, loadPosts, savePosts, updatePost, type Post, type Track } from "@/lib/social";
...
export default function ConnectPage() {
  ...
  const togglePlay = (post: Post) => {
    ...
    if (playingId === post.id) return stopAudio();
    stopAudio();
    const audio = new Audio(post.track.url);
  ...
  const submit = () => {
    ...
    if (sheet === "edit" && editingId) setPosts(updatePost(editingId, { text: text.trim(), photo, track, allowReplies }));
    else { createPost({ text: text.trim(), photo, track, allowReplies }); setPosts(loadPosts()); }
  ...
  const filteredPosts = useMemo(() => posts.filter((post) => tab === "proof" ? Boolean(post.photo) || post.text.toLowerCase().includes("proof") : true), [posts, tab]);
  ...
  return (
    <main className="relative min-h-full overflow-hidden pb-28">
      ...
      <section className="relative z-10 mx-auto max-w-xl px-4 pt-2">
        <div className="mb-3 flex items-center justify-between gap-3"><div className="flex min-w-0 items-center gap-2 overflow-x-auto pb-1 scrollbar-none">{PULSE.map((person) => <div key={person.username} className="shrink-0 text-center"><div className="mx-auto mb-1 flex h-11 w-11 items-center justify-center rounded-full border border-white/10 bg-white/[0.035]" style={{ boxShadow: `0 0 18px ${person.accent}22` }}><span className="text-[10px] font-bold" style={{ color: person.accent }}>{person.name.slice(0, 1)}</span></div><span className="text-[9px] text-white/45">{person.name}</span></div>)}</div><Link href="/home/share" className="shrink-0 rounded-full border border-white/10 bg-white/[0.035] px-3 py-2 text-[9px] font-semibold uppercase tracking-[0.15em] text-white/45">Share</Link></div>

        <div className="mb-4 flex items-center gap-1 overflow-x-auto rounded-2xl border border-white/[0.07] bg-white/[0.025] p-1">
          {(["live", "following", "proof", "watch"] as FeedTab[]).map((item) => <button key={item} type="button" onClick={() => setTab(item)} className={cn("shrink-0 rounded-xl px-4 py-2 text-[10px] font-semibold uppercase tracking-[0.14em]", tab === item ? "bg-white/[0.08] text-white" : "text-white/35")}>{item === "watch" ? "Watch" : item}</button>)}
        </div>

        {tab === "watch" ? <div className="space-y-3 snap-y snap-mandatory">
          {watchPosts.map((post) => <article key={post.id} className="relative min-h-[72vh] snap-start overflow-hidden rounded-[30px] border border-white/[0.08] bg-[#0a0c11]">
            {post.video ? <video src={post.video} className="absolute inset-0 h-full w-full object-cover" playsInline controls loop muted /> : post.photo ? <img src={post.photo} alt="" className="absolute inset-0 h-full w-full object-cover" /> : <div className="absolute inset-0 bg-gradient-to-br from-white/[0.07] via-transparent to-black"><div className="flex h-full items-center justify-center p-8 text-center"><p className="max-w-sm text-3xl font-semibold leading-tight text-white/85">{post.text}</p></div></div>}
