  ...
  return (
    <main className="relative min-h-full overflow-hidden pb-28">
      ...
      <section className="relative z-10 mx-auto max-w-xl px-4 pt-2">
        ...
        <div className="mb-4 flex items-center gap-1 rounded-2xl border border-white/[0.07] bg-white/[0.025] p-1">
          {(["live", "following", "proof"] as FeedTab[]).map((item) => <button key={item} type="button" onClick={() => setTab(item)} className={cn("flex-1 rounded-xl px-3 py-2 text-[10px] font-semibold uppercase tracking-[0.14em]", tab === item ? "bg-white/[0.08] text-white" : "text-white/35")}>{item}</button>)}
        </div>
        <button type="button" onClick={() => setSheet("compose")} className="mb-5 flex w-full items-center gap-3 rounded-2xl border border-white/[0.08] bg-white/[0.025] p-3 text-left"><Avatar identity={me ?? { displayName: "You", username: "me", photo: null, accent: "#4C8DFF" }} size={38} /><span className="text-[12px] text-white/35">Share something real...</span><span className="ml-auto rounded-full bg-white/10 px-3 py-1 text-[9px] font-semibold text-white/55">POST</span></button>
