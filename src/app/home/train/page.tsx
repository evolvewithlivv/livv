...
export default function TrainPage() {
    ...
    <main className="livv-page relative min-h-full overflow-hidden pb-10 pt-5">
      <Container className="relative z-10">
        <PageHero
          eyebrow="Workout"
          title="Train"
          subtitle="Pick a plan, then start."
          accent="#ff6b91"
        />
        ...
        {last && (
          <button
            type="button"
            onClick={repeatLast}
            className="mt-6 flex w-full items-center justify-between rounded-[28px] border border-livv-accent/30 bg-livv-accent/[0.12] px-5 py-5 text-left transition active:scale-[0.99]"
          >
            <div>
            ...
            <span className="flex h-12 w-12 items-center justify-center rounded-2xl bg-white text-black text-lg font-semibold">
              →
            </span>
          </button>
        )}
        ...
        <section className="mt-8">
          <div className="mb-4">
            <p className="text-[10px] uppercase tracking-[0.28em] text-white/35">1 · Split</p>
            <p className="mt-1 text-sm text-white/65">How your week is divided.</p>
          </div>
        ...
        {split && <section className="mt-8">
          <div className="mb-4">
            <p className="text-[10px] uppercase tracking-[0.28em] text-white/35">2 · Today</p>
            <p className="mt-1 text-sm text-white/65">Which day are you training?</p>
          </div>
          ...
          {selectedDay && <div className="mt-3 rounded-[22px] border border-white/10 bg-white/[0.035] p-4">
            <div className="flex items-center justify-between"><p className="text-[9px] uppercase tracking-[0.24em] text-white/30">Selected</p><p className="text-[9px] uppercase tracking-[0.2em]" style={{ color: split.color }}>{selectedDay.day}</p></div>
            <p className="mt-2 text-lg font-medium">{selectedDay.label}</p>
            <p className="mt-1 text-xs text-white/40">{selectedDay.muscles}</p>
            {selectedDay.rest && <p className="mt-3 text-xs text-white/35">Rest day. Pick a training day when you're ready.</p>}
          </div>}
        </section>}

        {split && selectedDay && !selectedDay.rest && <>
          <section className="mt-8">
            <div className="mb-4"><p className="text-[10px] uppercase tracking-[0.28em] text-white/35">3 · Where</p><p className="mt-1 text-sm text-white/65">Gym, home, or outside?</p></div>
          ...
          <section className="mt-7">
            <div className="mb-4"><p className="text-[10px] uppercase tracking-[0.28em] text-white/35">4 · How long</p><p className="mt-1 text-sm text-white/65">Pick a duration.</p></div>
        ...
        <div className="mt-9">
          <button type="button" disabled={!canGenerate} onClick={handleGenerate} className={"w-full rounded-full py-4 text-sm font-semibold transition " + (canGenerate ? "livv-breathe bg-white text-black" : "bg-white/10 text-white/30")} style={canGenerate ? { boxShadow: "0 0 36px rgba(255,255,255,0.45), 0 0 80px rgb(var(--livv-accent) / 0.35)" } : undefined}>
            {canGenerate ? `Build ${selectedDay?.label} workout` : split ? "Pick a training day" : "Pick a split to start"}
          </button>
        </div>
      </Container>
    </main>
  ...
  if (phase === "preview" && workout) return (
    <main className="relative min-h-full overflow-hidden pb-10 pt-5"><Container className="relative z-10">
      <button type="button" onClick={() => setPhase("select")} className="mb-5 text-[10px] uppercase tracking-[0.22em] text-white/35">← Change plan</button>
      <div className="rounded-[32px] border border-white/10 bg-white/[0.05] p-6 backdrop-blur-xl">
        <p className="text-[10px] uppercase tracking-[0.28em] text-livv-accent-soft">Your workout</p>
      ...
      <div className="mt-8"><Button variant="accent" size="lg" className="w-full" onClick={handleStart}>Start workout</Button></div>
    </Container></main>
  );
  ...
  if (phase === "rest" && workout) {
    ...
    return <main className="relative flex min-h-full flex-col items-center justify-center overflow-hidden pb-10"><Container className="relative z-10"><div className="text-center"><p className="text-[10px] uppercase tracking-[0.32em] text-livv-accent-soft">Rest</p><div className="mx-auto mt-7 flex h-56 w-56 items-center justify-center rounded-full border border-white/10 bg-white/[0.025]" style={{ background: `conic-gradient(rgb(255 255 255 / .8) ${pct}%, rgb(255 255 255 / .05) ${pct}%)` }}><div className="flex h-48 w-48 flex-col items-center justify-center rounded-full bg-[#090909]"><span className="text-6xl font-semibold tracking-tight">{restLeft}</span><span className="mt-1 text-[9px] uppercase tracking-[0.3em] text-white/30">seconds</span></div></div><p className="mt-7 text-lg font-medium">Rest up.</p><p className="mt-2 text-sm text-white/35">Next exercise next.</p><Button variant="secondary" className="mt-8" onClick={() => { if (restRef.current) window.clearInterval(restRef.current); setPhase("session"); }}>Skip rest</Button></div></Container></main>;
  }
  ...
  if (phase === "session" && workout) {
    ...
    return <main className="relative flex min-h-full flex-col overflow-hidden pb-8 pt-5"><Container className="relative z-10 flex flex-1 flex-col"><div className="flex items-center justify-between"><div><p className="text-[10px] uppercase tracking-[0.28em] text-white/30">Workout</p><p className="mt-1 text-xs text-white/45">{workout.name}</p></div><span className="text-[10px] uppercase tracking-[0.2em] text-livv-accent">{currentIndex + 1}/{workout.exercises.length}</span></div><div className="mt-4 h-1 overflow-hidden rounded-full bg-white/[0.08]"><div className="h-full rounded-full bg-livv-accent transition-all duration-500" style={{ width: `${progress}%` }} /></div><div className="flex flex-1 flex-col justify-center py-8"><div className="text-center"><p className="text-[10px] uppercase tracking-[0.35em] text-livv-accent-soft">Now</p><h1 className="mt-4 text-[2.7rem] font-semibold leading-[.98] tracking-[-0.05em]">{current.name}</h1><div className="mx-auto mt-7 h-px w-16 bg-white/15" /><p className="mt-6 text-lg text-white/60">{current.sets && `${current.sets} sets`}{current.reps && ` · ${current.reps}`}{current.duration && ` · ${current.duration}`}</p><p className="mt-2 text-xs uppercase tracking-[0.18em] text-white/25">Rest {current.rest} after</p></div></div><div className="space-y-3"><Button variant="accent" size="lg" className="h-14 w-full text-base" onClick={() => handleCompleteExercise(current)}>Done ✓</Button><Button variant="ghost" className="w-full" onClick={handleReset}>End workout</Button></div></Container></main>;
  }

  return <main className="relative min-h-full overflow-hidden pb-10 pt-5">{showMoment && <Moment title="Workout done" subtitle={workout ? `${workout.name} logged` : undefined} onDone={() => setShowMoment(false)} />}<Container className="relative z-10"><div className="pt-6 text-center"><div className="mx-auto flex h-20 w-20 items-center justify-center rounded-full border border-livv-accent/40 bg-livv-accent/10 text-2xl shadow-[0_0_45px_rgba(255,255,255,0.15)]">✓</div><h1 className="mt-6 text-3xl font-semibold tracking-tight">Session complete</h1><p className="mt-2 text-sm text-white/40">{workout?.name}</p><div className="mt-8 space-y-3"><Button variant="accent" size="lg" className="w-full" onClick={() => { handleReset(); }}>Train Again</Button><Button variant="secondary" className="w-full" onClick={() => window.location.href = "/home/progress">View progress</Button></div></div></Container></main>;
}
