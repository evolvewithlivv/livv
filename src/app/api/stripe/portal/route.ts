...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!stripe) return json({ error: "Stripe not configured" }, { status: 503 });
    ...
    if (!admin) return json({ error: "Billing identity is not configured" }, { status: 503 });
    ...
    return json({ error: "Billing portal could not be opened. Try again in a moment." }, { status: 500 });
  }
}
