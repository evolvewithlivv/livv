...
export async function POST(req: NextRequest) {
  try {
    ...
    let body: { kind?: string; grade?: number; qty?: number; tier?: string; email?: string; username?: string };
    try {
    ...
    const customerEmail = isSupabaseServerConfigured()
    ...
    if (body.kind === "pack") {
      ...
      if (!GRADES.includes(grade)) return json({ error: "Invalid pack" }, { status: 400 });
        ...
        line_items: [
          {
            ...
            price_data: {
              ...
              product_data: { name: GRADE_META[grade].name, description: shop.value },
            },
          },
        ],
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      subscription_data: {
        metadata: { livv_tier: tier, livv_username: usernameMeta, livv_user_id: livvUserId || "" },
      },
      allow_promotion_codes: true,
    });
    return json({ url: session.url, sessionId: session.id });
